...
def get_args():
    ...
    parser.add_argument('--mask_ratio', default=0.5, type=float,
                    help='Mask ratio (fixed, default: 0.5)')
    parser.add_argument('--temperature', default=0.8, type=float,
                        help='Softmax temperature for mask sampling (<1: sharper, >1: flatter, default: 0.8)')

    parser.add_argument('--use_importance_mask', action='store_true', default=True,
                        help='Use importance-based masking (mask high-info patches)')
    parser.add_argument('--use_random_mask', action='store_false', dest='use_importance_mask',
                        help='Use random masking instead')

    parser.add_argument('--max_continuous_mask', default=5, type=int,
                        help='Maximum continuous mask length (default: 5)')
    parser.add_argument('--punch_ratio', default=0.1, type=float,
                        help='Random punching ratio in masked regions (default: 0.1)')
...
def main(args):
    ...
    AGGREGATED_H5_DIR = Path("path")
    
    train_h5_files = sorted(AGGREGATED_H5_DIR.glob("TUEG_*.hdf5"))
    val_h5_files = sorted(AGGREGATED_H5_DIR.glob("TUEG_*.hdf5"))

    print(f"Found {len(train_h5_files)} training files")
    print(f"Found {len(val_h5_files)} validation files")
    
    train_datasets = [utils.H5AggregatedDataset(h5_path) for h5_path in train_h5_files]
    
    from torch.utils.data import ConcatDataset
    dataset_train = ConcatDataset(train_datasets)
    dataset_train_list = [dataset_train]
    
    train_ch_names_list = [train_datasets[0].get_ch_names()]
