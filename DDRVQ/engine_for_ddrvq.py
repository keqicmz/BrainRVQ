...
def train_one_epoch(model: torch.nn.Module, 
    ...
    print("Resetting codebook statistics for training...")
    if hasattr(model.module, 'quantize_t'):
        model.module.quantize_t.reset_cluster_size(device)
        print("Reset quantize_t cluster size.")
    
    if hasattr(model.module, 'quantize_f'):
        model.module.quantize_f.reset_cluster_size(device)
        print("Reset quantize_f cluster size.")
    ...
    if hasattr(model.module, 'quantize_t'):
        q_t = model.module.quantize_t
        unused_per_layer_t = [(cs == 0).sum().item() for cs in q_t.cluster_size]

        for idx, num in enumerate(unused_per_layer_t):
            print(f"[quantize_t] Layer {idx}: unused code = {num}")
            train_stat[f'Unused_code_T_L{idx}'] = num

    if hasattr(model.module, 'quantize_f'):
        q_f = model.module.quantize_f
        unused_per_layer_f = [(cs == 0).sum().item() for cs in q_f.cluster_size]

        for idx, num in enumerate(unused_per_layer_f):
            print(f"[quantize_f] Layer {idx}: unused code = {num}")
            train_stat[f'Unused_code_F_L{idx}'] = num
...
def evaluate(data_loader_list, model, device, log_writer=None, epoch=None, ch_names_list=None, args=None):
    ...
    print("Resetting codebook statistics for validation...")
    if hasattr(model.module, 'quantize_t'):
        model.module.quantize_t.reset_cluster_size(device)
        print("Reset quantize_t cluster size.")
    ...
    if hasattr(model.module, 'quantize_t'):
        ...
        for idx, num in enumerate(unused_per_layer_t):
            print(f"[quantize_t] Layer {idx}: unused code = {num}")
            test_stat[f'Unused_code_T_L{idx}'] = num
    ...
    if hasattr(model.module, 'quantize_f'):
        ...
        for idx, num in enumerate(unused_per_layer_f):
            print(f"[quantize_f] Layer {idx}: unused code = {num}")
            test_stat[f'Unused_code_F_L{idx}'] = num
